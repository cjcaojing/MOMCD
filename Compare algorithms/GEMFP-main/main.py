...
if __name__ == '__main__':
    ...
    for i in range(1):
        time_begin = time.time()
        m, G ,t= get_newMatrix("./Data/realnetwork/"+net+".txt")
        print("特征矩阵计算完成")
    
        # label_file = "./Data/realnetwork/real/"+net+"_groundtruth.txt"
        Feature_Propagation(G,m,0.001)
    
        label = prepare(G)
        time_end= time.time()
        runtime=time_end-time_begin
        G = nx.read_edgelist("./Data/realnetwork/"+net+".txt")
        G = G.to_undirected()  # 转换成无向图
        n = G.number_of_nodes()  # 获取一个Graph对象中node的数量
        # 基于这些连边使用igraph创建一个新网络
        Gi = ig.Graph.Read_Edgelist("./Data/realnetwork/"+net+".txt")
        ...
        # nmi_list.append(nmi)
        print(net)
        print(f"{np.mean(qw_list):.3f}"+'('+f"{np.var(qw_list):.2e}"+')')

        # print(f"{np.mean(nmi):.3f}"+'('+f"{np.var(nmi):.2e}"+')')


        print(f"{np.mean(t_list):.3f}")
